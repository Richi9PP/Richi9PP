...
<p align="center">
  <a href="https://www.linkedin.com/in/patryk-piasecki-680a92231/">
    <img src="https://img.shields.io/badge/LinkedIn-Patryk%20Piasecki-0A66C2?logo=linkedin&logoColor=white"></a>
  <a href="mailto:patrykpiasecki9@gmail.com">
    <img src="https://img.shields.io/badge/Email-patrykpiasecki9@gmail.com-EA4335?logo=gmail&logoColor=white"></a>
</p>
...
| **[Real-Time Fraud Detection (P2P Payments)](https://github.com/Richi9PP/Projekt_RTA)** | Streaming pipeline that scores each transaction's fraud risk in **<200 ms**: generator → **Apache Kafka** → **Spark Structured Streaming** → ML risk model (**XGBoost** / Logistic Regression) → real-time alerts on **Grafana / InfluxDB**, orchestrated with **Docker Compose**. Detects account-takeover, smurfing, geo-anomaly and layering patterns. `Kafka` `Spark` `XGBoost` `Docker` `Python` |
| **[Olist Product Analytics in SQL](https://github.com/Richi9PP/olist-sql-analytics)** | Advanced **SQL** (window functions, `NTILE`, multi-table JOINs) on the real Olist e-commerce dataset (~100k orders): GMV growth, RFM segmentation, retention cohorts, seller Pareto. `SQL` `DuckDB` `Python` |
| **[FX Risk & Cost Dashboard](https://github.com/Richi9PP/Fx_risk_dashboard)** | Interactive dashboard on **real ECB exchange-rate data** (Frankfurter API): rolling **volatility**, correlations, **VaR / Expected Shortfall**, drawdowns, plus a **mid-market vs markup** conversion-cost simulator and a random-walk forecast benchmark. `Python` `Streamlit` `Plotly` `pandas` `pytest` |
...
| **[Real-Time Event Photo Streaming + AI](https://eventowemagnesy.onrender.com/upload?computer=Computer1&type=standard)** | Production full-stack system that streams photos from guests' phones (via QR) to on-site computers in **real time over WebSockets** — with auto-reconnection and **zero server-side storage** (privacy by design). Adds optional **AI photo enhancement** via the **Replicate API** behind an explicit **GDPR consent** flow. `Python` `Flask` `WebSockets` `Replicate API` `Render` |
